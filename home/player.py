...
# I will be using methods from my spaceobject file so I'm bringing parts of it here
class MySpaceObject:
...
class Player:
    ...
    def move_to_clos_ast(self, spaceship, asteroid_ls):
        ...
        for asteroid in asteroid_ls:
            ast_dist[asteroid] = MySpaceObject.dist_to_ast(self, spaceship, asteroid)
    ...
    def shoot_ast(self, spaceship):
        ...
        if MySpaceObject.dist_to_ast(self, spaceship, self.current_ast) < (config.bullet_move_count * config.speed['bullet']) / 1.3:
    ...
    def avoid_ast(self, spaceship, asteroid_ls, fuel):
        ...
        if fuel <= config.shoot_fuel_threshold:
            ...
            fake_spaceship = MySpaceObject(spaceship.x, spaceship.y, spaceship.width, spaceship.height, spaceship.angle, spaceship.obj_type, spaceship.id)        
            MySpaceObject.predict_move(fake_spaceship)
            ...
            for asteroid in asteroid_ls:
                ...
                fake_asteroid = MySpaceObject(asteroid.x, asteroid.y, spaceship.width, spaceship.height, asteroid.angle, asteroid.obj_type, asteroid.id)
                MySpaceObject.predict_move(fake_asteroid)
                
                # If the spaceship is going to collide with an asteroid, tell it to change its trajectory
                if MySpaceObject.collide_with(fake_spaceship, fake_asteroid):
                    if self.left == True:
                        self.left = False
                        self.right = True
                    if self.right == True:
